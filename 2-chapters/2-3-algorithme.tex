...
On déclare qu'une suite de champs de croix converge vers un autre champ de croix lorsque les champs d'angles associés convergent les uns vers les autres.

Pour comparer une fonction $\eta_h$ définie sur $\Omega_h$ à une fonction définie sur $\Omega$, nous introduisons une opération d'extension, appelée \emph{lift de} $\eta$ et notée $[\mathbf{L}_{\Omega_h}^{\Omega}\eta_h]$. À cette fin, nous supposons que $\Omega$ peut être représenté globalement par une fonction de distance orientée $d$ définie sur un sous-ensemble ouvert $U$ de $\mathbb{R}^2$.
$$\Omega=\{x\in U,~d(x)=0\},$$
$d$ dans $\mathcal{C}^{k,\alpha}(U)$, $\nabla d\neq 0$ ($k\in\mathbb{N}\cup\{0\},~0\leq\alpha\leq 1$). Le lift $[\mathbf{L}_{\Omega_h}^{\Omega}\eta_h]$ est alors défini par:
\begin{equation}
\eta_h(p)=[\mathbf{L}_{\Omega_h}^{\Omega}\eta_h](p-d(p)n(p))\quad\quad\quad pour~tout~p\in\Omega_h
\end{equation}
où $n$ est la normale sortante en $p$.
...
$$
\[\]
Les lignes de niveau des champs scalaires $U$ et $V$ forment ainsi une grille régulière à partir de laquelle on peut déduire un maillage pour le bloc $\Gamma$. Pour ce faire, il suffit de considérer les lignes de niveau correspondant aux valeurs prises par les nœuds de discrétisation sur les bords de ces champs. Cela est illustré dans les figures \ref{fig:quad_equation_1}, \ref{fig:quad_equation_2} et \ref{fig:quad_equation_3}. Dans cet exemple, nous choisissons $m=n=12$. Les intersections des lignes de niveaux sont alors considérées comme des nœuds du maillage, et les éléments du maillage sont construits en reliant ces nœuds. Cependant, un tel maillage peut être insatisfaisant car on observe que les cellules ne sont pas homogènes. De plus, les noeuds du bords ne correspondent pas forcément au discrétrisation des séparatrices ce qui entraîne un déphase des noeuds entre deux blocs voisins et donc une non-conformité du maillage. Pour remédier à cela, nous introduisons une grille régulière notée $(u_{ij}){\substack{i\in\llbracket 1, n\rrbracket\j\in\llbracket 1, m\rrbracket}}$ de taille $n\times m$, dont les nœuds $u{ij}$ sont donnés par:
\begin{equation}
...
Dans les sections précédentes, nous avons examiné le problème du partitionnement d'un domaine défini par un maillage triangulaire, ainsi que celui de la génération d'un maillage quadrangulaire à partir du partitionnement construit. Dans cette partie, nous montrons que le maillage construit est bien un maillage du domaine de départ $\Omega$. Pour se faire il suffit de montrer que le partitionnement construit sur $\Omega_h$ que nous notons $P_h$ converge vers un partitionnement de $\Omega$ ayant des partitions de quatre côtés.

Pour commencer, remarquo


Cependant, une condition cruciale pour le bon déroulement de ce processus est que tous les blocs issus du partitionnement possèdent quatre côtés (en d'autres termes, les bords des blocs doivent résulter de l'intersection de quatre séparatrices distinctes). La question qui se pose alors est de savoir qu'est ce qui garantit que le processus de partitionnement aboutit bien a un partitionnement en blocs de quatre côtés. La réponse à cette interrogation a été exposée dans le chapitre \ref{chap:theoritical}, à travers les théorèmes \ref{thm:theorem1}, \ref{thm:theorem2} et \ref{thm:theorem3}. Ces théorèmes établissent la garantie du processus de partitionnement lorsqu'il est appliqué à un champ de croix presque-$\mathcal{C}^1$, aligné par rapport au bord du domaine où il est défini.

On veut montrer que le maillage produit est bien un maillage quadrilatéral de $\Omega$. Pour cela, il suffit de montrer que les partitionnement $P_h$ tendent vers un partitionnement de $\Omega$ ayant des partions de quatres côtés.

$v_h$ tend vers $v$ qui partitionne $\Omega$ en régions de quatre côtés. Notons $P$ ce partitionnement.

Nous allons donc montrer que le partitionnement $P_h$ construit sur $\Omega$ converge vers $P$ ce qui nous permet de dire que pour $h$ assez fin, $P_h$ sera un partitionnement de quatre côtés. $P_h$ ayant été construit à partir des séparatrices de $v_h$, il suffit donc de montrer que $v_h$ converge vers les séparatrices de $v$.

$\Omega_h$ tend vers $\Omega$ donc nh tend vers n. le champ phih est une approximation element fini du probleme continu:

Sachant que Lnh tend vers n sur Omega, il vient que Lphih tend vers phi sur Omega. Par conséquent Lthetavh converge vers thetav sur Omega et par suite vh tend vers v.
$v_h$ tend vers $v$. en effet,
\begin{lemma}
Le champ de croix $\bar{v}_h$ converge vers le champ de croix $\bar{v}$ vérifiant:
...
L'ensemble des points singuliers de $v_h$ tend vers l'ensemble des points singuliers de $v$

Les séparatrices interne de $v_h$ convergent vers les séparatrices de $v$: en effet, l'ensemble des points singuliers de $v_h$ converge vers l'ensemble des points singuliers de $v$ et les directions de sortie des séparatrices de $v_h$ converge vers les directions de sorties des séparatrices de $v$. Ce la découle directement du fait que $v_h$ converger vers $v$ lorsque $h$ tend vers $0$.

\begin{corollary}
$\mathcal{S}_{\bar{v}_h}\rightarrow \mathcal{S}_{\bar{v}}$\\
$\sum id_{\bar{v}_h}= \sum id_{\bar{v}}$\\
$W_{\bar{u}_h}^h\rightarrow W_{\bar{u}}$ donc Convergence de dirh vers dir\\
Convergence de Sinth vers Sinth\\
...
\[\]
Convergence de Ph vers P\\
Convergence de singh vers sing\\
Convergence de Sbordh vers Sbord


\[\]
Pour simplifier, nous supposerons que $Cl = Cl_h$. Cette condition signifie en particulier que $Cl$ est un domaine de frontière polygonale (cf. Figure 2).

\[\]
remarque les quad de bord on pour base les triangles de bord, mesh quad plus fin que le tri
motivation, reconstitué la courbure de
\color{red}
Hermitte récupere de la courbure
\color{black}
Pour finir il faut que le bord soit une separatrice

\section{Reconstitution du bord du domaine}
Remarquons que dans ce que nous avons présenté jusqu'a présent, les mailles quadrilatéral construit sur le bord du domaine ont leur noeuds déposé sur les aretes de bord du maillage triangulaire ayant servi à la construction. Cependant, dans les codes de calcul, on pourrait être amené a interpolé la courbure sur la frontière du maillage quadrialtéral. c'est par exemple le cas lorsqu'on a des conditions limites qui font intervenir la dérivée normale. face a une telle situation les calculs de reconstitution de normal seront donc dependant de la resolution obtenu pour les mailles de bord lors de la construction du maillage quadrilatéral. Ainsi, si le pas demandé pour le maillage quadrilatéral est plus fin que le pas du maillage triangulaire sur lequel se base la construction, on ne pourra pas récupérer  fait avec les noeuds de bord du maillage quadrilatéral et donc plus le bord
pouvoir expliciter omega tilde permet de se passer des triangles de bord et de ne plus etre assujeti
...
Comme dit dans l'introduction \ref{thesis_target} les champs de croix générer en propageant la normale extérieur à l'intérieur du domaine \cite{kowalski2013pde} produisent une distribution de singularités pouvant conduire à un partitionnement invalide ou non souhaité. Par exemple le champ de croix présenté sur la figure ... Un problème notable survient dans les domaines extrêmement étirés, tandis qu'une autre préoccupation concerne les formes de partitionnement qui entraînent des tailles de cellules très non uniformes (voir figure \ref{fig:dom_etire_mesh_inhomogene}).
Comme
