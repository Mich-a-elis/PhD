...
\begin{itemize}
 ...
 \left\{
 \begin{array}{l}
 \bar{u}_h(p)=\displaystyle\left\{\mathbf{R}\left(\theta_{\bar{u}_h}(p)+m\frac{\pi}{2}\right)(1,0)^t,~m\in\mathbb{Z}\right\},\\\\
 \theta_{\bar{u}_h}(p)=\theta_{\bar{u}_h}(s_1)+\displaystyle\frac{\|\overrightarrow{s_1p}\|}{\|\overrightarrow{s_1s_2\|}\delta\theta(\bar{u}_h(s_1),\bar{u}_h(s_2).
 \end{array}
 \right.
 $$
 $\bar{u}_h(p)=\bar{u}(p)$,\\[-0.2cm]
