...
\color{red}
$u_h$ tend vers $u$ quand $h$ tend vers $0$ ???\\
$\theta_h$ linéaire dans chaque triangle non singulier donc $u_h$ tend vers $u$ dans ces triangles\\
Dans les triangles non singulier.....
\color{black}
\[\]
Un conséquence de la représentation du champ de croix que nous avons présenté est que les points singuliers de $u$ ne correspondent pas aux points singuliers de $u$ ($\mathcal{S}_{\bar{u}}=\mathcal{S}_{\bar{u}_h}$). \color{red} Cependant la somme des indices des points singuliers de $u$ est équivalent à la somme des indices des points singuliers de $\bar{u}_h$.\color{black}.
\[\]
Par exemple, dans la figure \ref{fig:eclatement_point_sing}, nous disposons d'un champ de croix comprenant un point singulier d'indice $1$ représenté sur un maillage triangulaire. On observe dans le champ de croix présenté la présence d'une multitude de points singuliers d'indice $1/4$ et d'indices $-1/4$, localisés dans divers triangles. Il est à noter que la somme des indices de ces points singuliers correspond bien à l'indice du point singulier initial.
...
Considérons $T$ comme un triangle singulier que doit traverser une séparatrice dont l'origine ne se trouve pas dans $T$. On commence en subdivisant $T$ en trois triangles en reliant les milieux des arêtes de $T$.

\begin{comment}
\begin{figure}[!h]
\centering
\includegraphics[scale=0.24]{images/draw_streams_sing_1.pdf}\hfill
\includegraphics[scale=0.24]{images/draw_streams_sing_2.pdf}\vspace{1cm}
\includegraphics[scale=0.24]{images/draw_streams_sing_3.pdf}\hfill
\includegraphics[scale=0.24]{images/draw_streams_sing_4.pdf}
\caption{Illustration du maillage quadrilatéral d'un anneau à partir d'un champ de croix radial.}
...
Une fois les séparatrices construites, le maillage triangulaire $\Omega_h$ est divisé en plusieurs régions. Pour identifier ces régions, nous modifions $\Omega_h$ en y insérant les segments constituants les séparatrices construites générant ainsi un nouveau maillage. L'objectif est de récupérer chaque région sous la forme d'un sous-maillage de $\Omega_h$. Cette opération peut être réalisée localement dans chaque triangle du maillage $\Omega_h$, ce qui permet en pratique de bénéficier des capacités de calcul parallèle pour accélérer le processus.

Considérons $T$ comme un triangle de $\Omega_h$ traversé par certains segments des séparatrices construites. Pour insérer ces segments dans $T$, nous commençons par introduire les points de bord de ces segments tout en conservant la structure en triangles du triangle initial. De cette manière, nous obtenons un maillage triangulaire pour $T$. En utilisant des opérations de retournement d'arêtes, nous cherchons ensuite à retrouver les segments constituant les séparatrices ayant traversé $T$. Cette procédure s'inspire de la génération de maillages selon la méthode de type Voronoi présentée dans \cite{georgegeneration}. Notons $K$\\
Pour insérer un point $p$ dans $T$, on commence par chercher tout l'élément ou les deux éléments de $T$ contenant $p$ puis on insère $p$ dans ses éléments tout en générant de nouveaux triangles de la manière suivante:\\
\begin{itemize}
 \item si $p$ se trouve à l'intérieur d'un triangle $K$ alors on insère $p$ dans $T$ en créant trois nouveaux triangles en reliant $p$ aux sommets de $K$\\
 \item si $p$ se trouve sur une arête d'un triangle $K$ de $T$, on inère $p$ dans $T$ en créant deux nouveaux \ref{fig:insert_pt}
\end{itemize}
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.58]{images/decoup_triangle.pdf}\hfill
\includegraphics[scale=0.58]{images/decoup_triangle.pdf}
\caption{Direction de sortie différentes.}
\label{fig:insert_pt}
\end{figure}

 Trouver l'élément ou les deux éléments de $T$ contenant $p$.
Insérer $p$ dans ces éléments
\[\]
en faisant:
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.28]{images/eclatement_1.pdf}
\hfill
\includegraphics[scale=0.28]{images/eclatement_2.pdf}
\\[1cm]
\includegraphics[scale=0.45]{images/eclatement_3.pdf}
\caption{Direction de sortie différentes.}
...
Faire plusieurs cercles avec different size mesh pour montrer la difference entre avec hermitte et sans hermitte
