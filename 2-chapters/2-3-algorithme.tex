...
On déclare qu'une suite de champs de croix converge vers un autre champ de croix lorsque les champs d'angles associés convergent les uns vers les autres. Pour comparer une fonction $\eta_h$ définie sur $\Omega_h$ à une fonction définie sur $\Omega$, nous introduisons une opération d'extension, appelée \emph{lift de} $\eta$ et notée $[\mathbf{L}_{\Omega_h}^{\Omega}\eta_h]$. À cette fin, nous supposons que $\Omega$ peut être représenté globalement par une fonction de distance orientée $d$ définie sur un sous-ensemble ouvert $U$ de $\mathbb{R}^2$.
$$\Omega=\{x\in U,~d(x)=0\},$$
