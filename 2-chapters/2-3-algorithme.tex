...
\begin{itemize}
 \item pour chaque arête $e\in E$ ayant des sommets $s_1$ et $s_2$, soit $\bar{u}(s_1)\neq 0$ soit $\bar{u}(s_2)\neq 0$. De plus, si à la fois $\bar{u}(s_1)\neq 0$ et $\bar{u}(s_2)\neq 0$, alors $\delta\theta(\bar{u}(s_1), \bar{u}(s_2))$ est défini.\\[-0.2cm]
 \item pour chaque arête $e\in E$ ayant des sommets $s_1$ et $s_2$, soit $\bar{u}(s_1)\neq 0$ soit $\bar{u}(s_2)\neq 0$. De plus, si à la fois $\bar{u}(s_1)\neq 0$ et $\bar{u}(s_2)\neq 0$, alors $\delta\theta(\bar{u}(s_1), \bar{u}(s_2))$ est défini.\\[-0.2cm]
\end{itemize}
Dans la pratique, il sera donc impératif d'affiner ou de modifier localement un maillage qui ne satisfait pas ces contraintes. L'importance de ces contraintes prend tout son sens par la suite, avec l'approche de construction que nous proposons pour représenter $\bar{u}$ sur $\Omega_h$. Nous introduisons maintenant la notion de triangle singulier via la définition suivante:
...
\begin{itemize}
 ...
 $$
 %\\[-0.2cm]
 \item[$\bullet$] sinon, soit $T$ un triangle de $\Omega_h$ de sommets $s_1, s_2$ et $s_3$ tel que $p\in T$. On distingue trois cas:\\[-0.2cm]
 \begin{itemize}
  \item si on a
  $$
  \left\{
  \begin{array}{l}
  (\bar{u}_h(s_1), \bar{u}_h(s_2), \bar{u}_h(s_3))\neq (0, 0, 0),\\\\
  \displaystyle\sum_{i\in\llbracket 1, 3\rrbracket}\delta\theta(\bar{u}_h(s_i), \bar{u}_h(s_{i+1}))=0~~(\mbox{avec } s_4:=s_1),
  \end{array}
  \right
  $$
  alors on pose
...
\end{lemma}
\begin{proof}
    Soit $p$ un point singulier de $\bar{u}_h$. Le point $p$ est isolé puisque par construction, on a $F_p\cap\mathcal{S}_{\bar{u}_h}=\{p\}$. En effet, pour tout $q\in F_p\backslash\{p\}$ on a $\bar{u}_h(q)=$ pour tout $p\in\mathcal{S}_{\bar{u}_h}$, F
