...
$$
\mathbf{c}_2=\mathbf{c}_1+\delta\theta(\mathbf{c}_1,\mathbf{c}_2).
$$
Nous dirons que $\delta\theta(\mathbf{c}_1,\mathbf{c}_2$) n'est pas défini si $|\theta_{\mathbf{c}_2}-\theta_{\mathbf{c}_1}|=\pi/4$.\\\\
Nous représentons le champ de croix $\bar{u}$ sur $\Omega_h$ par un champ de croix noté $\bar{u}_h$, dont la définition est exposée dans les lignes suivantes. On se donne un ensemble de valeurs de $\bar{u}$ défini sur les sommets de $\Omega_h$ et on défini pour tout $p\in V$, $\bar{u}_h(p)=\bar{u}(p)$. On suppose alors que pour chaque arête $e\in E$ avec des sommets $s_1$ et $s_2$ tels que $\bar{u}_h(s_1)\neq 0$ et $\bar{u}_h(s_2)\neq 0$, $\delta\theta(\bar{u}_h(), \bar{u}_h())$ soit défini.
