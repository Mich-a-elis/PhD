...
Nous représentons le champ de croix $\bar{u}$ sur $\Omega_h$ par un champ de croix noté $\bar{u}_h$, dont la définition est exposée dans les lignes suivantes. On se donne un ensemble de valeurs de $\bar{u}$ défini sur les sommets de $\Omega_h$ et on définit pour tout $p\in V$, $\bar{u}_h(p)=\bar{u}(p)$.
